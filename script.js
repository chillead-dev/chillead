(() => {
  ...
  // gif whitelist (users can only add these via picker)
  const GIFS = {
  ...
  function initGifPicker() {
    ...
    document.querySelectorAll(".gif-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        selectedGifId = btn.getAttribute("data-g");
        // visual feedback: outline selected
        document.querySelectorAll(".gif-btn").forEach(b => b.style.outline = "none");
  ...
  function buildMessage(text, gifId) {
    // encode gif as a tag the backend will store in plain message:
    // user text + \n[g1]
    // This avoids backend changes and remains safe.
    const t = String(text || "").trim();
    const g = gifId ? String(gifId) : "";
    if (g) return (t ? t + "\n" : "") + `[g${g}]`;
    return t;
  }
  ...
  async function submitLetter() {
    ...
    if (text.length < 1 && !selectedGifId) return;

    btn.disabled = true;
    if (st) st.textContent = "sending…";

    const message = buildMessage(text, selectedGifId);
  ...
  // -------- shoutbox (safe render) --------
  let approvedCache = [];
  let currentPage = 1;

  function clearNode(n){ while(n && n.firstChild) n.removeChild(n); }

  function parseMessageToNodes(message) {
    // Safe:
    // - no innerHTML
    // - render [g1]..[g4] tags as gif from whitelist
    // - render text lines as text nodes
    const frag = document.createDocumentFragment();
